...
def concatenate_images(front=False, side=False, all_around=False):
	...
	top_down_image = Image.open("simulations/{0}/xy/{0}_{1}.png".format(simulation_name, str(snapshots[snapshot_id][-9:-5]).zfill(snapshot_number_sig_fig)))
	
	if front or all_around:
		front_on_image = Image.open("simulations/{0}/xz/{0}_{1}.png".format(simulation_name, str(snapshots[snapshot_id][-9:-5]).zfill(snapshot_number_sig_fig)))		
	if side or all_around:
		side_on_image = Image.open("simulations/{0}/yz/{0}_{1}.png".format(simulation_name, str(snapshots[snapshot_id][-9:-5]).zfill(snapshot_number_sig_fig)))	
	...
	if front:
		return
		#combined_output_name = "simulations/{0}/xy_xz/{0}_{1}.png".format(simulation_name, str(snapshots[snapshot_id][-9:-5]).zfill(snapshot_number_sig_fig))
	if side:
		combined_output_name = "simulations/{0}/xy_yz/{0}_{1}.png".format(simulation_name, str(snapshots[snapshot_id][-9:-5]).zfill(snapshot_number_sig_fig))
	if all_around:
		combined_output_name = "simulations/{0}/xy_xz_yz/{0}_{1}.png".format(simulation_name, str(snapshots[snapshot_id][-9:-5]).zfill(snapshot_number_sig_fig))
...
for snapshot_id in range(start_index, end_index):
	...
	if top:
		...
		pos, vel, m, mat_id, ax_lim, id, tracked_id = snapshot.get_pos_vel_m_mat_id(snapshots[snapshot_id], ax_lim, num_target_particles, tracked_index=tracked_index, tracked_id=tracked_id)
	...
	if front:
		...
		pos, vel, m, mat_id, ax_lim, id, tracked_id = snapshot.get_pos_vel_m_mat_id_side(snapshots[snapshot_id], ax_lim, num_target_particles, tracked_index=tracked_index, tracked_id=tracked_id, side="front", full=full)
	...
	if side:
		## While we could just use the same data again, reading the data in again allows us to treat it differently if we want.
		## One thing that will always be necessary is to reorder the data so that the front most particles are drawn on top of the back most, which is handled here, so we might as well just do it all again
		pos, vel, m, mat_id, ax_lim, id, tracked_id = snapshot.get_pos_vel_m_mat_id_side(snapshots[snapshot_id], ax_lim, num_target_particles, tracked_index=tracked_index, tracked_id=tracked_id, side="side", full=full)
